...
void Bloup() {                    // Обработчик Блютуза
  ...
  switch (in_telephone) {
    case 52: {
        Hellou();
#ifdef debug_bluet
        Serial.println("Verstein Hellou");
#endif debug_bluet
        break;
      }
    case 53: {
    ...
    case 62: {
      ...
      //    case 58: {
      //#ifdef debug_bluet
      //        Serial.println("Verstein RGB");
      //#endif debug_bluet
      //        Swet_up();
      //        break;
      //      }
      //    case 59: {
      //        flag_bri_down = !flag_bri_down;
      //#ifdef debug_bluet
      //        Serial.print("Bri down: ");
      //        Serial.println(flag_bri_down);
      //#endif debug_bluet
      //        break;
      //      }
      //    case 60: {
      //        flag_bri_up = !flag_bri_up;
      //#ifdef debug_bluet
      //        Serial.print("Bri up: ");
      //        Serial.println(flag_bri_up);
      //#endif debug_bluet
      //        break;
  }
}
...
void GoodNight() {
  ...
  Serial.println("Хозяин, когда изволите просыпаться?");

  //Serial.setTimeout(100500);
  while (!Serial.available()) {}
  set_hour = Serial.parseInt();
  while (!Serial.available()) {}
  set_minute = Serial.parseInt();
  ...
  Serial.print("Ваше время пробуждения установлено на: "); Serial.print(String(set_hour) + ":"); Serial.println(set_minute);
  Serial.println("Приятных снов");
...
void RFID() {
  com transmition;
  transmition.comand = chg_mode;
  transmition.data[0] = 8;          // Режим Пока

  if (digitalRead(buttonRFID)) {
    if (bri != 0) {
      while (bri != 0) {
        bri_down();
      }
...
void Real_Time(){
  clock.read();
  Serial.println(String(clock.getHour() + ":" + (clock.getMinute())));
...
void bri_up() {
  if (bri != 224) {
    bri++;
#ifdef debug_serial_bloet
    Serial.println("up");
#endif debug_serial_bloet
    strip.setBrightness(bri);
  }
}
// Функция Огня
...
void Zason() {                    // Алгоритм Засыпания
  ...
  // двигаем пламя
  //  fireTick();
  mp3.stop();     //запускаем mp3 с паузы
}
